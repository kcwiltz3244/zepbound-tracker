...
/* Version 13.0 nutrition storage bridge.
   Version 12.2 saved food logs under mzjV81Nutrition. Version 13 uses a
   canonical key while retaining the legacy key for safe rollback. */
const NUTRITION_STORAGE_KEY="mzjV13Nutrition";
const LEGACY_NUTRITION_STORAGE_KEYS=["mzjV81Nutrition"];
const NUTRITION_SAFETY_SNAPSHOT_KEY="mzjV1331NutritionSafetySnapshot";
const NUTRITION_SAFETY_MARKER_KEY="mzjV1331NutritionIntegrityChecked";

function nutritionEntrySignature(entry){
  if(entry?.id)return `id:${entry.id}`;
  return [entry?.date||"",entry?.meal||"",entry?.name||entry?.food||"",entry?.amount||entry?.quantity||"",entry?.unit||"",entry?.calories||0,entry?.protein||0,entry?.carbs||0,entry?.fat||0].join("|").toLowerCase();
}
function mergeNutritionCopies(copies){
  const merged=[],seen=new Set();
  for(const entries of copies){
    if(!Array.isArray(entries))continue;
    for(const entry of entries){
      if(!entry||typeof entry!=="object")continue;
      const signature=nutritionEntrySignature(entry);
      if(seen.has(signature))continue;
      seen.add(signature);merged.push(entry);
    }
  }
  return merged.sort((a,b)=>String(a.date||"").localeCompare(String(b.date||"")));
}
function nutritionDateList(entries){
  return [...new Set((entries||[]).map(e=>e?.date).filter(Boolean))].sort();
}
function makeNutritionSafetySnapshot(copies,merged){
  if(localStorage.getItem(NUTRITION_SAFETY_MARKER_KEY))return;
  const snapshot={
    createdAt:new Date().toISOString(),
    reason:"Version 13.3.1 pre-merge food-journal safety snapshot",
    sources:copies.map(c=>({key:c.key,entryCount:Array.isArray(c.entries)?c.entries.length:0,dates:nutritionDateList(c.entries),entries:Array.isArray(c.entries)?c.entries:[]})),
    mergedEntryCount:merged.length,
    mergedDates:nutritionDateList(merged)
  };
  localStorage.setItem(NUTRITION_SAFETY_SNAPSHOT_KEY,JSON.stringify(snapshot));
  localStorage.setItem(NUTRITION_SAFETY_MARKER_KEY,JSON.stringify({checkedAt:snapshot.createdAt,entryCount:merged.length,dates:snapshot.mergedDates}));
}
function loadNutritionEntries(){
  const copies=[
    {key:NUTRITION_STORAGE_KEY,entries:read(NUTRITION_STORAGE_KEY,null)},
    ...LEGACY_NUTRITION_STORAGE_KEYS.map(key=>({key,entries:read(key,null)}))
  ];
  const merged=mergeNutritionCopies(copies.map(c=>c.entries));
  makeNutritionSafetySnapshot(copies,merged);
  // Rewrite both copies only after the original versions have been snapshotted.
  localStorage.setItem(NUTRITION_STORAGE_KEY,JSON.stringify(merged));
  for(const key of LEGACY_NUTRITION_STORAGE_KEYS)localStorage.setItem(key,JSON.stringify(merged));
  return merged;
}
function persistNutritionEntries(entries){
  const safeEntries=mergeNutritionCopies([entries]);
  write(NUTRITION_STORAGE_KEY,safeEntries);
  // Dual-write so a rollback cannot hide food records.
  for(const key of LEGACY_NUTRITION_STORAGE_KEYS)localStorage.setItem(key,JSON.stringify(safeEntries));
}
...
{name:"Green beans, fresh, cooked",aliases:["green beans","fresh green beans","string beans","snap beans","vegetable"],servingAmount:1,servingUnit:"cup",servingLabel:"1 cup cooked",calories:44,protein:2.4,carbs:10,sugar:3.5,fiber:4, fat:.4,sodium:2},
{name:"Green beans, canned, drained",aliases:["green beans","canned green beans","string beans"],servingAmount:1,servingUnit:"cup",servingLabel:"1 cup drained",calories:36,protein:1.8,carbs:8,sugar:3,fiber:3.5,fat:.2,sodium:350},
{name:"Cucumber, raw, sliced",aliases:["cucumber","fresh cucumber","cucumber slices","vegetable"],servingAmount:1,servingUnit:"cup",servingLabel:"1 cup sliced",calories:16,protein:.7,carbs:3.8,sugar:1.7,fiber:.5,fat:.1,sodium:2},
{name:"Cucumber salad",aliases:["cucumber","cucumber salad","marinated cucumber"],servingAmount:1,servingUnit:"cup",servingLabel:"1 cup",calories:70,protein:1,carbs:10,sugar:6,fiber:1.5,fat:3,sodium:300},
{name:"Smoked sausage, pork and beef",aliases:["sausage","smoked sausage","beef sausage","pork sausage","link sausage"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:290,protein:12,carbs:4,sugar:1,fiber:0,fat:25,sodium:780},
{name:"Turkey sausage link",aliases:["sausage","turkey sausage","breakfast sausage","sausage link"],servingAmount:2,servingUnit:"piece",servingLabel:"2 links",calories:120,protein:10,carbs:2,sugar:1,fiber:0,fat:8,sodium:500},
{name:"Breakfast sausage, pork",aliases:["sausage","breakfast sausage","pork sausage","sausage patty"],servingAmount:2,servingUnit:"oz",servingLabel:"2 oz cooked",calories:190,protein:9,carbs:1,sugar:0,fiber:0,fat:17,sodium:450},
{name:"Italian sausage, cooked",aliases:["sausage","italian sausage","pork sausage"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:260,protein:14,carbs:2,sugar:1,fiber:0,fat:22,sodium:650},
{name:"Watermelon, diced",aliases:["watermelon","melon","fruit"],servingAmount:1,servingUnit:"cup",servingLabel:"1 cup diced",calories:46,protein:.9,carbs:11.5,sugar:9.4,fiber:.6,fat:.2,sodium:2},
{name:"Dymatize ISO100 protein powder",aliases:["dymatize","iso100","protein powder","vanilla protein"],servingAmount:1,servingUnit:"serving",servingLabel:"1 scoop",calories:120,protein:25,carbs:2,sugar:1,fiber:0,fat:.5,sodium:160}


,{name:"Beef steak, cooked, average",aliases:["steak","beef steak","beef","cooked steak","grilled steak","pan fried steak"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:210,protein:25,carbs:0,sugar:0,fiber:0,fat:12,sodium:55}
,{name:"Sirloin steak, cooked",aliases:["steak","sirloin","sirloin steak","top sirloin","beef","grilled sirloin"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:207,protein:26,carbs:0,sugar:0,fiber:0,fat:11,sodium:55}
,{name:"Ribeye steak, cooked",aliases:["steak","ribeye","rib eye","ribeye steak","beef","grilled ribeye"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:248,protein:22,carbs:0,sugar:0,fiber:0,fat:18,sodium:58}
,{name:"New York strip steak, cooked",aliases:["steak","new york strip","ny strip","strip steak","beef","grilled strip steak"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:220,protein:24,carbs:0,sugar:0,fiber:0,fat:14,sodium:55}
,{name:"Filet mignon, cooked",aliases:["steak","filet mignon","filet","beef tenderloin","tenderloin steak","beef"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:218,protein:24,carbs:0,sugar:0,fiber:0,fat:13,sodium:52}
,{name:"T-bone steak, cooked",aliases:["steak","t bone","t-bone","t bone steak","porterhouse","beef"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:225,protein:23,carbs:0,sugar:0,fiber:0,fat:15,sodium:57}
,{name:"Beef chuck roast, cooked",aliases:["beef","roast beef","chuck roast","pot roast","beef roast","cooked beef"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:240,protein:24,carbs:0,sugar:0,fiber:0,fat:16,sodium:65}
,{name:"Beef stew meat, cooked",aliases:["beef","stew beef","beef stew meat","cubed beef","cooked beef"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:215,protein:25,carbs:0,sugar:0,fiber:0,fat:12,sodium:60}
,{name:"Ground beef, 80/20, cooked",aliases:["ground beef","80/20 ground beef","cooked ground beef","hamburger meat","beef cooked"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:230,protein:21.5,carbs:0,sugar:0,fiber:0,fat:15.5,sodium:75}
,{name:"Ground beef, 85/15, cooked",aliases:["85/15 ground beef","lean ground beef","cooked ground beef"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:213,protein:22,carbs:0,sugar:0,fiber:0,fat:13,sodium:72}
,{name:"Ground beef, 90/10, cooked",aliases:["90/10 ground beef","lean ground beef","extra lean ground beef"],servingAmount:3,servingUnit:"oz",servingLabel:"3 oz cooked",calories:184,protein:22.5,carbs:0,sugar:0,fiber:0,fat:10,sodium:72}
,{name:"Cabot Whole Milk Greek Yogurt, Plain 10%",aliases:["cabot yogurt","cabot greek yogurt","whole milk greek yogurt","10 percent greek yogurt","plain greek yogurt"],servingAmount:.75,servingUnit:"cup",servingLabel:"3/4 cup (170 g)",calories:230,protein:14,carbs:9,sugar:6,fiber:0,fat:16,sodium:65}
];
...
function saveNutrition(){persistNutritionEntries(nutritionEntries);renderNutrition()}
...
function renderNutritionSearch(query=""){
  ...
  const q=normalize(query),terms=q.split(" ").filter(Boolean);
  const scored=NUTRITION_FOODS.map((f,index)=>{const name=normalize(f.name),aliases=(f.aliases||[]).map(normalize),haystack=[name,...aliases].join(" ");let score=0;if(!q)score=1;else{if(name===q)score+=100;if(aliases.includes(q))score+=90;if(name.startsWith(q))score+=50;if(haystack.includes(q))score+=30;terms.forEach(t=>{if(name.split(" ").includes(t))score+=12;else if(haystack.includes(t))score+=5})}return {f,index,score}}).filter(x=>x.score>0).sort((a,b)=>b.score-a.score||a.index-b.index);
  const results=scored.slice(0,16).map(x=>x.f);
...
const V10_CLOUD_CONFIG_KEY="mzjV13CloudConfig";
let v10OnlineFoods=[];
let v10UsdaFoods=[];
let v10OffFoods=[];
...
function v10CloudConfig(){
  try{return JSON.parse(localStorage.getItem(V10_CLOUD_CONFIG_KEY)||"{}")||{};}catch{return {};}
}
async function v10SearchUsda(query){
  const config=v10CloudConfig();
  if(!config.apiUrl||!config.token)throw new Error("USDA search needs Cloud setup");
  const url=`${String(config.apiUrl).replace(/\/$/,"")}/api/foods/search?q=${encodeURIComponent(query)}&pageSize=30`;
  const response=await fetch(url,{headers:{Accept:"application/json",Authorization:`Bearer ${config.token}`}});
  if(!response.ok){let message=`USDA search returned ${response.status}`;try{message=(await response.json()).error||message}catch{}throw new Error(message);}
  const data=await response.json();
  return (Array.isArray(data.foods)?data.foods:[]).filter(v10FoodHasNutrition);
}
async function v10SearchOpenFoodFacts(query){
  const params=new URLSearchParams({
    search_terms:query,search_simple:"1",action:"process",json:"1",page_size:"24",
    fields:"product_name_en,product_name,generic_name_en,generic_name,brands,nutriments,image_front_small_url,image_small_url"
  });
  ...
  const data=await response.json();
  return (Array.isArray(data.products)?data.products:[]).map(v10ProductToFood).filter(food=>food&&v10FoodHasNutrition(food));
}
function v10UniqueFoods(foods,limit=20){
  return foods.filter((food,index,array)=>array.findIndex(other=>normalizeFoodText(other.name)===normalizeFoodText(food.name))===index).slice(0,limit);
}
...
function v10FoodSearchScore(food,query){
  const q=normalizeFoodText(query);if(!q)return -1;
  const name=normalizeFoodText(food.name);
  const aliases=(food.aliases||[]).map(normalizeFoodText);
  const haystacks=[name,...aliases];
  let score=-1;
  if(name===q)score=120;
  ...
  else if(aliases.some(a=>a.includes(q)))score=75;
  else {
    const words=q.split(' ').filter(Boolean);
    const matched=words.filter(w=>haystacks.some(h=>h.includes(w))).length;
    if(matched)score=45+matched*8;
    if(q.length>=4&&haystacks.some(h=>h.split(' ').some(w=>v10EditDistance(w,q)<=1)))score=Math.max(score,65);
  }
  if(score<0)return score;
  const packaged=/canned|jarred|bottled|pickled|preserved|in sauce|with sauce|brand|restaurant/i.test(food.name);
  const everyday=/fresh|raw|plain|cooked|steamed|baked|grilled|roasted/i.test(food.name);
  if(everyday)score+=18;
...
async function v10SearchOnline(){
  ...
  const query=(input?.value||"").trim();
  if(query.length<2){if(status)status.textContent="Type at least two letters first.";input?.focus();return;}

  const local=v10LocalMatches(query);
  v10RenderCombinedResults(query,null);
  if(status)status.textContent="Searching USDA and packaged-food databases…";
  if(button){button.disabled=true;button.textContent="Searching…";}

  let usdaError=null,offError=null;
  try{
    const results=await Promise.allSettled([v10SearchUsda(query),v10SearchOpenFoodFacts(query)]);
    if(results[0].status==="fulfilled")v10UsdaFoods=v10UniqueFoods(results[0].value,24);else{v10UsdaFoods=[];usdaError=results[0].reason;}
    if(results[1].status==="fulfilled")v10OffFoods=v10UniqueFoods(results[1].value,16);else{v10OffFoods=[];offError=results[1].reason;}
    v10OnlineFoods=[...v10UsdaFoods,...v10OffFoods];

    const wrap=document.getElementById("nutritionFoodResults");
    if(wrap){
      if(v10UsdaFoods.length)wrap.insertAdjacentHTML("beforeend",`<p class="result-section-label online-results-label">USDA FoodData Central</p>${v10UsdaFoods.map((f,i)=>v10FoodCard(f,i,true)).join("")}`);
      if(v10OffFoods.length){const offset=v10UsdaFoods.length;wrap.insertAdjacentHTML("beforeend",`<p class="result-section-label online-results-label">Packaged foods from Open Food Facts</p>${v10OffFoods.map((f,i)=>v10FoodCard(f,i+offset,true)).join("")}`);}
      v10BindFoodCards(wrap);
    }
    const total=v10OnlineFoods.length;
    if(status){
      if(total)status.textContent=`Found ${local.length} built-in, ${v10UsdaFoods.length} USDA, and ${v10OffFoods.length} packaged-food result${v10OffFoods.length===1?"":"s"}.`;
      else if(usdaError&&/Cloud setup/i.test(String(usdaError.message||usdaError)))status.textContent="Built-in results are shown. Connect Cloud setup and add the USDA key to enable the complete USDA search.";
      else status.textContent=local.length?"Built-in matches are shown, but the online databases returned no reliable results.":`No reliable match was found for “${query}.”`;
    }
  }catch(error){
    console.warn("Online food search unavailable",error);
    if(status)status.textContent=local.length?"Built-in matches are shown. Online food search is temporarily unavailable.":"Online food search is temporarily unavailable. You can still enter nutrition manually.";
  }finally{if(button){button.disabled=false;button.textContent="Search all foods";}}
}
...
function v10InitSmartFoodSearch(){
  ...
  if(badge)badge.textContent="USDA + packaged foods · 13.3";
...
function v11AddDiningItem(item){
  ...
  persistNutritionEntries(nutritionEntries);
  renderNutrition();
...
async function putProgressPhoto(key,blob){
  ...
  db.close();
  try{window.MZJFoundation?.recordPhotoChange?.(key,blob,"upsert")}catch(err){console.warn("Photo sync journal update failed",err)}
}
...
async function deleteProgressPhoto(key){
  ...
  db.close();
  try{window.MZJFoundation?.recordPhotoChange?.(key,null,"delete")}catch(err){console.warn("Photo sync journal update failed",err)}
}
...
(function(){
  ...
  async function getAllPhotos(){try{const db=await openPhotos();const result=await new Promise((resolve,reject)=>{const tx=db.transaction(PHOTO_DB_STORE,"readonly"),store=tx.objectStore(PHOTO_DB_STORE),keysReq=store.getAllKeys(),valuesReq=store.getAll();tx.oncomplete=async()=>{try{const out=[];for(let i=0;i<keysReq.result.length;i++){const blob=valuesReq.result[i];if(blob)out.push({key:String(keysReq.result[i]),type:blob.type||"application/octet-stream",data:await blobToDataURL(blob)})}resolve(out)}catch(err){reject(err)}};tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error)});db.close();return result}catch(err){recordError(err,"backup-photos");return []}}
  ...
  async function renderDiagnostics(){const d=await diagnosticSnapshot(),grid=document.getElementById("v13DiagGrid"),errors=document.getElementById("v13ErrorList");if(!grid)return;const rows=[["App version",d.version],["Release channel",d.channel],["Connection",d.online?"Online":"Offline"],["Cloud configured",d.configured?"Yes":"No"],["Cloud status",d.cloudStatus||"Not configured"],["Pending changes",d.pendingChanges],["Data sections",d.dataSections],["Saved photos",d.photos],["Local storage",d.localStorage],["Last synchronization",d.lastSync?new Date(d.lastSync).toLocaleString():"Never"],["Offline cache",d.serviceWorker?"Active":"Waiting"]];grid.innerHTML=rows.map(([a,b])=>`<article><small>${a}</small><strong>${esc(b)}</strong></article>`).join("");errors.innerHTML=d.errors.length?d.errors.slice(0,5).map(e=>`<div><strong>${new Date(e.at).toLocaleString()}</strong><span>${esc(e.context)}: ${esc(e.message)}</span></div>`).join(""):"<p>No recent application errors recorded.</p>"}
  function statusText(){const m=getMeta(),p=(safeParse(localStorage.getItem(JOURNAL_KEY),[])||[]).filter(x=>x.status==="pending").length;if(!configured())return "Cloud connection needs setup";if(!navigator.onLine)return `Offline · ${p} change${p===1?"":"s"} safely stored`;if(m.cloud.status==="syncing")return "Synchronizing laptop and phone…";if(m.cloud.status==="error")return `Sync error · ${p} pending`;return `${m.cloud.status==="synced"?"Synced":"Ready"} · ${p} pending change${p===1?"":"s"}`}
