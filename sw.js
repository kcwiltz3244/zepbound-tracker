...
const CORE_ASSETS = ["./", "./index.html?v=13.1.8", "./styles.css?v=13.1.8", "./v13-controls.js?v=13.1.8", "./v13-navigation.js?v=13.1.8", "./app.js?v=13.1.8", "./manifest.json?v=13.1.8", "./photo-label-guide.png"];
...
self.addEventListener("fetch",event=>{
  ...
  if(isCode){
    event.respondWith(fetch(event.request,{cache:"no-store"}).then(response=>{if(response&&response.ok){const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(event.request,copy));}return response;}).catch(()=>caches.match(event.request).then(hit=>hit||caches.match("./index.html?v=13.1.8"))));
    return;
  }
